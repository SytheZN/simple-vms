...
internal sealed class MotionGridH265Worker : IDataStream<MotionGridUnit>, IAsyncDisposable
{
  private bool _disposed;
  private readonly Guid _cameraId;
  private readonly string _parentProfile;
  private readonly IDataStream<H265NalUnit> _input;
  private readonly Channel<MotionGridUnit> _output;
  private readonly H265SkipExtractor _extractor;
  ...
  public MotionGridH265Worker(
    Guid cameraId, string parentProfile,
    IDataStream<H265NalUnit> input, MotionGridProcessor processor, ILogger logger)
  {
    _cameraId = cameraId;
    ...
    _logger = logger;
    _extractor = new H265SkipExtractor(logger);
  ...
  private async Task RunAsync(CancellationToken ct)
  {
    ...
    {
      _logger.LogError(ex, "MotionGridH265Worker failed for camera {CameraId} profile '{Profile}'",
